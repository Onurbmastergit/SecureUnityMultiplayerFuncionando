...
public class LevelManager : NetworkBehaviour
{
    ...
    void HourTick()
    {
        ...
        if (currentHour.Value == 24)
        {
            ...
            ShowSyncCalendar(currentDay); // Atualiza atrav�s do Observer, para todos
            SetHour(0);
            //isDay = false;
            //SyncDay(isDay);
        }
        ...
        ShowSyncHour(currentHour.Value); // Atualiza atrav�s do Observer, para todos
    ...
    // Como o client n�o tem acesso �s informa��es do que aconteceu no server, � necess�rio passar a
    // refer�ncia do server para o server, vai toma no cu Bruno.
    [ObserversRpc(BufferLast = true)]
